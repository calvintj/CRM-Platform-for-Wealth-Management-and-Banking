...
const getTotalAUM = async (rm_number, customerRisk) => {
  if (customerRisk === "all") {
    const result = await db.query(`
      SELECT 
        ra.rm_number,
        SUM(CASE WHEN ci.risk_profile = '1 - Conservative' THEN ca.fum ELSE 0 END) AS conservative_aum,
        SUM(CASE WHEN ci.risk_profile = '2 - Balanced' THEN ca.fum ELSE 0 END) AS balanced_aum,
        SUM(CASE WHEN ci.risk_profile = '3 - Moderate' THEN ca.fum ELSE 0 END) AS moderate_aum,
        SUM(CASE WHEN ci.risk_profile = '4 - Growth' THEN ca.fum ELSE 0 END) AS growth_aum,
        SUM(CASE WHEN ci.risk_profile = '5 - Aggressive' THEN ca.fum ELSE 0 END) AS aggressive_aum
      FROM rm_account AS ra
      JOIN customer_info AS ci ON ci.assigned_rm = ra.rm_number
