...
const getTopProducts = async (rm_number) => {
    const result = await db.query(
      `
...
const getCertainCustomerList = async (rm_number, customerRisk) => {
...
WHERE ra.rm_number = '${rm_number}' AND ci.risk_profile = '${customerRisk}'
GROUP BY 
